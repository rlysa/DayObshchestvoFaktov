...
def profiles_like_me(user_id):
    ...
    ids_lm = cursor.execute(f'''SELECT sbd_likes_user FROM likes WHERE user_id="{user_id}"''').fetchall()[0][0].split()
    ...
    profiles_lm = [list(cursor.execute(f'SELECT user_id, name, sex, age, city, description FROM users WHERE user_id={user_id_i}').fetchall()[0]) for user_id_i in ids_lm_new]
